...
export default function Dashboard() {
  ...
  const [dashboardSnapshot, setDashboardSnapshot] = useState<{ openIncidents: number; vulnerabilities: Array<{ severity: string; _count: { _all: number } }>; risks: Array<{ _count: { _all: number } }>; frameworks: Array<{ readinessPercent: number }> } | null>(null);
  ...
  useEffect(() => { api.get<typeof dashboardSnapshot>('/api/dashboard').then(setDashboardSnapshot).catch(() => setDashboardSnapshot(null)); }, []);
  useEffect(() => {
    ...
    <main data-theme={theme} style={{ '--canvas': currentTheme.canvas, '--surface': currentTheme.surface, '--ink': currentTheme.ink, '--line': currentTheme.line, '--teal': currentTheme.accent, '--profile-accent': currentTheme.accentSoft, '--accent-ink': currentTheme.ink, '--muted-surface': currentTheme.accentSoft, '--highlight': currentTheme.highlight, '--highlight-ink': currentTheme.highlightInk, '--sidebar': currentTheme.sidebar, '--muted': currentTheme.muted } as React.CSSProperties} className="dashboard-app min-h-screen bg-[var(--canvas)] text-[var(--ink)]">
      ...
      <section className="lg:pl-[260px]">
        ...
        <div className="mx-auto max-w-[1500px] p-5 lg:p-10">
          ...
          {showSecurityOverview && <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[['Security incidents', String(dashboardSnapshot?.openIncidents || 0), 'Live data', 'coral'], ['Critical / high vulnerabilities', String((dashboardSnapshot?.vulnerabilities || []).filter(item => item.severity === 'CRITICAL' || item.severity === 'HIGH').reduce((sum, item) => sum + item._count._all, 0)), 'Live data', 'red'], ['Open risks', String((dashboardSnapshot?.risks || []).reduce((sum, item) => sum + item._count._all, 0)), 'Live data', 'amber'], ['Security compliance', dashboardSnapshot?.frameworks.length ? `${Math.round(dashboardSnapshot.frameworks.reduce((sum, item) => sum + item.readinessPercent, 0) / dashboardSnapshot.frameworks.length)}%` : '—', 'Live data', 'mint']].map(([label, value, trend, color]) => <article key={label} className="panel relative overflow-hidden p-4"><div className="flex justify-between text-[11px] text-[#8ca2a4]"><span>{label}</span><span className={`rounded-md px-2 py-1 ${color === 'mint' ? 'bg-[var(--muted-surface)] text-[var(--teal)]' : color === 'amber' ? 'bg-[#493b26] text-[var(--amber)]' : 'bg-[#492b33] text-[var(--coral)]'}`}>◆</span></div><strong className="mt-3 block font-display text-3xl">{value}</strong><div className="mt-1 text-[10px] text-[var(--teal)]">{trend}</div><div className="mt-4 h-1 rounded bg-[#18353b]"><i className="block h-full w-0 rounded bg-[var(--teal)]" /></div></article>)}</div>}
...
function WorkspaceDashboard({ notify, onNavigate, onAddUser, onUploadDocument, onBuildReport }: { notify: (text: string) => void; onNavigate: (name: string) => void; onAddUser: () => void; onUploadDocument: () => void; onBuildReport: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try { const rows = await api.get<InventoryDevice[]>('/api/inventory'); assets = rows.length; } catch { assets = 0; }
      try { const rows = await api.get<BackupRecord[]>('/api/backups'); backups = rows.length; } catch { backups = 0; }
      try { const rows = await api.get<Array<{ id: number }>>('/api/users'); members = rows.length; } catch { members = 0; }
      try { const rows = await api.get<Array<{ status: string }>>('/api/records?module=Security%20Incidents'); openIncidents = rows.filter(incident => incident.status !== 'Resolved').length; } catch { openIncidents = 0; }
      if (!cancelled) setStats({ workItems: backups + openIncidents, assets, members, openIncidents });
    })();
    return () => { cancelled = true; };
  }, []);
  const tasks: Array<[string, string, string]> = stats.workItems ? [['Open records requiring review', 'Audit & Findings', 'Current'], ['Open security incidents', 'Security Incidents', 'Current']] : [];
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><WorkspaceMetric label="Open work items" value={String(stats.workItems)} detail={`${stats.openIncidents} incidents open`} accent="var(--teal)" /><WorkspaceMetric label="Registered assets" value={String(stats.assets)} detail="In inventory" accent="var(--blue)" /><WorkspaceMetric label="Team members" value={String(stats.members)} detail="User accounts" accent="var(--amber)" /><WorkspaceMetric label="Platform uptime" value="—" detail="No monitoring data" accent="var(--coral)" /></div>
...
function DashboardPanels({ onNavigate, activity }: { onNavigate: (name: string) => void; activity: Array<[string, number, string]> }) {
  ...
  const riskRows: Array<[string, number, string, string]> = ['Critical', 'High', 'Medium', 'Low'].map((label, index) => {
    ...
    return [label, riskTotal ? Math.round((count / riskTotal) * 100) : 0, ['#ed6b73', '#ef855b', '#f5b55e', '#4f8b93'][index], String(count)];
  });
  return <div className="grid gap-3 xl:grid-cols-3">
    <article className="panel p-5 xl:col-span-2"><Header title="Security incidents" subtitle="Incident volume by current status" action="Live database snapshot" /><div className="grid h-48 grid-cols-4 items-end gap-5 border-b border-[var(--line)] bg-[linear-gradient(to_bottom,transparent_0%,transparent_24%,#17343b_25%,transparent_26%,transparent_49%,#17343b_50%,transparent_51%,transparent_74%,#17343b_75%,transparent_76%)] px-5">{barHeights.map((height, index) => <div key={index} className="flex h-full flex-col items-center justify-end gap-2"><div className="flex h-full items-end gap-1"><i className="w-3 rounded-t bg-[var(--teal)]" style={{ height: `${height}%` }} /><i className="w-3 rounded-t bg-[var(--blue)]" style={{ height: `${height - 25}%` }} /><i className="w-3 rounded-t bg-[#8d8af7]" style={{ height: `${height - 43}%` }} /></div><small className="text-[9px] text-[#617477]">{['Current', 'Current', 'Current', 'Current'][index]}</small></div>)}</div><div className="mt-4 flex flex-wrap items-center gap-4 text-[10px] text-[#8ca2a4]"><span>● Open <b className="text-white">{snapshot?.openIncidents || 0}</b></span><button onClick={() => onNavigate('Security Incidents')} className="ml-auto text-[var(--teal)]">View incidents →</button></div></article>
    <article className="panel p-5"><Header title="Vulnerabilities" subtitle="Current findings by severity" /><div className="my-8 flex items-center gap-5"><div className="grid h-32 w-32 shrink-0 place-items-center rounded-full" style={{ background: 'conic-gradient(#ed6b73 0 25%,#ef855b 25% 50%,#f5b55e 50% 75%,#4f8b93 75% 100%)' }}><div className="grid h-20 w-20 place-items-center rounded-full bg-[var(--surface)]"><strong className="font-display text-xl">{vulnerabilityTotal}</strong></div></div><div className="space-y-3 text-[10px] text-[#8ca2a4]">{['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'].map(severity => <p key={severity}>● {severity[0] + severity.slice(1).toLowerCase()} <b className="text-white">{snapshot?.vulnerabilities.find(row => row.severity === severity)?._count._all || 0}</b></p>)}</div></div><button onClick={() => onNavigate('Vulnerability Management')} className="text-[10px] text-[var(--teal)]">View vulnerability register →</button></article>
    <article className="panel p-5"><Header title="Compliance posture" subtitle="Framework readiness score" /><div className="my-7 flex justify-around">{(snapshot?.frameworks || []).slice(0, 4).map((framework, index) => <div key={framework.name} className="text-center"><div className="grid h-20 w-20 place-items-center rounded-full border-4 border-[#1d3b42]" style={{ borderTopColor: ['var(--teal)', 'var(--blue)', 'var(--amber)', '#8d8af7'][index] }}><strong className="font-display text-lg">{framework.readinessPercent}%</strong></div><small className="mt-2 block text-[9px] text-[#617477]">{framework.name}</small></div>)}</div><div className="border-t border-[var(--line)] pt-3 text-[10px] text-[#617477]">● <span className="text-[var(--teal)]">{snapshot?.frameworks.length ? 'Live readiness from evidence' : 'No certification data'}</span></div></article>
    <article className="panel p-5 xl:col-span-2"><Header title="Risk register" subtitle="Risk distribution by rating" action="Live database snapshot" /><div className="space-y-4 py-4">{riskRows.map(([label, width, color, total]) => <div key={label} className="grid grid-cols-[60px_1fr_24px] items-center gap-3 text-[10px] text-[#8ca2a4]"><span>{label}</span><div className="h-2 rounded bg-[#1b363c]"><i className="block h-full rounded" style={{ width: `${width}%`, background: color }} /></div><b className="text-right text-white">{total}</b></div>)}</div><div className="border-t border-[var(--line)] pt-3 text-[10px] text-[#617477]"><strong className="font-display text-xl text-white">{riskTotal}</strong> total registered risks<button onClick={() => onNavigate('Risk Management')} className="float-right text-[var(--teal)]">Open register →</button></div></article>
    <article className="panel p-5"><Header title="Security activities" subtitle="Department completion" action="Live records" />{activity.length ? activity.map(([name, value, color]) => <div key={name} className="my-4"><div className="mb-2 flex justify-between text-[10px] text-[#a9bdbc]"><span>{name}</span><b>{value}%</b></div><div className="h-2 rounded bg-[#1b363c]"><i className={`block h-full rounded ${color === 'mint' ? 'bg-[var(--teal)]' : color === 'blue' ? 'bg-[var(--blue)]' : color === 'amber' ? 'bg-[var(--amber)]' : 'bg-[#bf8df5]'}`} style={{ width: `${value}%` }} /></div></div>) : <div className="p-6 text-xs text-[var(--muted)]">No security activities recorded.</div>}</article>
    <article className="panel p-5 xl:col-span-2"><Header title="Attention required" subtitle="Items needing ownership this week" action="Live records" /><div className="p-6 text-xs text-[var(--muted)]">Attention items are shown from live module records.</div><button onClick={() => onNavigate('Audit & Findings')} className="mt-4 text-[10px] text-[var(--teal)]">View all actions →</button></article>
    <article className="panel p-5"><Header title="Protected IT assets" subtitle="Coverage and health snapshot" /><div className="my-7 flex items-center justify-between"><div><strong className="font-display text-4xl">{snapshot?.assets || 0}</strong><span className="block text-[10px] text-[#617477]">registered assets</span></div><div className="text-center"><div className="grid h-16 w-16 place-items-center rounded-full border-4 border-[#1c4547] border-t-[var(--teal)]"><strong className="font-display">—</strong></div><span className="text-[10px] text-[#617477]">no coverage data</span></div></div><button onClick={() => onNavigate('Security Assets')} className="text-[10px] text-[var(--teal)]">Manage assets →</button></article>
  </div>;
}
