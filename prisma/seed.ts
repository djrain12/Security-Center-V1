...
async function main() {
  ...
  const masterAdmin = await prisma.user.upsert({
    where:{email:'master.admin@wsi.local'},
    update:{fullName:'Master Admin',accessLevel:AccessLevel.MASTER_ADMIN,roleId:roles[0].id,passwordHash:defaultPassword,status:'ACTIVE'},
    create:{fullName:'Master Admin',email:'master.admin@wsi.local',department:'Cyber Security',roleId:roles[0].id,accessLevel:AccessLevel.MASTER_ADMIN,contactNumber:'+63 917 555 0101',address:'WSI Main Office',passwordHash:defaultPassword}
  });
  const tara = await prisma.user.upsert({where:{email:'user@wsi.local'},update:{accessLevel:AccessLevel.IT_SECURITY_OFFICER},create:{fullName:'Security Officer',email:'user@wsi.local',department:'Cyber Security',roleId:roles[2].id,accessLevel:AccessLevel.IT_SECURITY_OFFICER,contactNumber:'+63 917 555 0102',address:'WSI Main Office',passwordHash:defaultPassword}});
  ...
  await prisma.securityIncident.upsert({where:{incidentCode:'INC-2026-041'},update:{},create:{incidentCode:'INC-2026-041',title:'Suspicious authentication pattern',severity:Severity.HIGH,status:IncidentStatus.INVESTIGATING,assignedTo:masterAdmin.id,affectedAssetId:identity.id,detectedAt:new Date()}});
  ...
  await prisma.securityActivity.createMany({data:[{name:'VAPT engagements',category:'VAPT',completionPercent:78,ownerId:masterAdmin.id},{name:'Patching cadence',category:'Patching',completionPercent:92,ownerId:tara.id},{name:'Access reviews',category:'Access',completionPercent:64,ownerId:masterAdmin.id},{name:'Backup verification',category:'Backup',completionPercent:100,ownerId:tara.id}]});
  ...
  await prisma.userModulePermission.createMany({data:modules.map(moduleName=>({userId:masterAdmin.id,moduleName,allowed:true})),skipDuplicates:true});
  await prisma.auditLog.create({data:{actorId:masterAdmin.id,action:'SEEDED_WORKSPACE',resourceType:'security_dashboard',metadata:{source:'prisma seed'}}});
